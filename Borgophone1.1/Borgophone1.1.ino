...
#define ON_Delay   3   // Set Delay after ON threshold before velocity is checked (wait for tounging peak)
...
#define BREATHCURVE_FACTORY 8     // 0 to 12 (-4 to +4, S1 to S4) ... 0 means you don't have to blow hard at all to get loud, 4 is straight line, 8 means you have to blow pretty hard before volume gets loud 9-12 are S curves that make it easy to blow quiet and loud, and don't favor middle volumes
...
#define OctUpPin 5  //
#define OctDnPin 4 //
#define ProgChgUpPin 8 //
#define ProgChgDnPin 7
#define MidChUpPin 3
#define MidChDnPin 2
#define ModePin 9 //
#define CurvePin 11
...
void setup() {
  ...
  pinMode(ModePin,INPUT_PULLUP); // Mode Selector +
  pinMode(CurvePin,INPUT_PULLUP); // Extra Button
...
//          Serial.println(fingeredNote);
      ...
      if (pressureSensor < ON_Thr) {
        ...
        if (instrumentMode >1){
            usbMIDI.sendNoteOff(activeNoteL, velocitySend, (MIDIchannel +1) ); // send Note On message for new note 
          }
        digitalWrite(13,LOW); 
        breathLevel=0;
        state = NOTE_OFF;
...
void breath(){
  bool breathCCselection = digitalRead(10);
...
void curveSelector(){
  ...
  pixels.setPixelColor(numberOfCurves, 60, 40, 0);
...
//  Serial.println(instrumentMode);
  ...
  switch (instrumentMode){
    case 0:// Tuba mode
      fingeredNote=constrain(startNote + (2*LH1 + LH2 + 4*LH3 + 7*LH4) - (2*RH1 + RH2 + 3*RH3 + 5*RH4 + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) 
      ,0, 127);
      break;
...
void readPots(){
  int pot1Reading = analogRead(A10);
