...
int lastFingering = 0;             // Keep the last fingering value for debouncingModeDebo
...
byte ccList[9] = {0,1,2,7,11};  // OFF, Modulation, Breath, Volume, Expression
int breathDeadZone = 60;
...
#define breathCCselector 10
#define OctUpPin 7
#define OctDnPin 8
#define MidChUpPin 3
#define MidChDnPin 2
#define CurvePin 11 
#define PitchBendPin A6
#define JoyY A7
#define ForceSensitiveResistor 23
#define pot1pin 22
#define pot2pin 14
...
#define ProgChgUpPin 12
#define ProgChgDnPin 9
...
unsigned long curveCounter = BREATHCURVE_FACTORY;
...
void setup() {
  ...
  pinMode(breathLED, OUTPUT);
  while (!Serial) { // needed to keep leonardo/micro from starting too fast!
    delay(10);
  }
  
  // Buttons & debouncing
  pinMode(breathCCselector, INPUT_PULLUP); // select between sending breath to CC2 or CC11
  ...
  pinMode(CurvePin,INPUT_PULLUP); // Mode Selector +
  ...
  CurvePinDebo.attach(CurvePin);
  CurvePinDebo.interval(debounceInterval); // interval in ms

  pinMode(ProgChgUpPin,INPUT_PULLUP); // Program change +
  pinMode(ProgChgDnPin,INPUT_PULLUP); // Program change -
  ProgChgupDebo.attach(ProgChgUpPin);
  ProgChgupDebo.interval(debounceInterval); // interval in ms
  ProgChgDnDebo.attach(ProgChgDnPin);
...
void mainLoop() {  
  ...
  while(1){
    ...
    if (ProgChgupDebo.fell() && ProgChangeNumber < 127 ){
      ...
      pixels.clear();
      pixels.setPixelColor(ProgChangeNumber % 12, pixels.Color(100, 100, 50));
    ...
    if (MidChUpDebo.fell()&& MIDIchannel < 16 ){
      MIDIchannel ++;
      pixels.clear();
      pixels.setPixelColor(MIDIchannel % 12, pixels.Color(5, 10, 20));
      Serial.println("Midi +");
    ...
    if (CurvePinDebo.fell() ){
      ...
      curveCounter++;
      curveSelector();
      pixels.setPixelColor(curve, 60, 20, 0);
...
//      }
//      pixels.clear();
//      pixels.setPixelColor(instrumentMode, 0, 20, 0);
//      modeSelector();
//      Serial.println("Mode");
//      Serial.println(instrumentModeCounter);
//    }

    breathFilter.input(analogRead(A11));
    pressureSensor = constrain((int)breathFilter.output(),3400,4095); // Get the filtered pressure sensor reading from analog pin A11, input from sensor MP3V5004GP
...
//          Serial.println(fingeredNote);
          ...
          state = NOTE_ON;
          if (millis() - breath_on_time*5 > ON_Delay){
      }
        }
      } else {
...
//        modeSelector();
        ...
        // than the debounce delay, so take it as the actual current state
          if (fingeredNote != activeNote) {
            ...
            // the new note.      
//            analogWrite(faceLED, pitchClass*10 +20);
            breathValHires = breathCurve(map(constrain(breathLevel,ON_Thr,breath_max),ON_Thr,breath_max,0,16383));
            velocitySend = (breathValHires >>7) & 0x007F;
...
//            breath(); // send breath data
    ...
    if (millis() - ccSendTime > CC_INTERVAL && (abs(pressureSensor -initial_breath_value) > breathDeadZone)) {
...
void breath(){
  bool breathCCselection = digitalRead(breathCCselector);
//  bool breathCCselection = HIGH;
  if(breathCCselection == HIGH) {
    breathCC = 2; //0x02 is standard breath control CC
  } else if (breathCCselection == LOW){
    breathCC = 0x04; //0x0B is good for Session Horns velocity
  }
...
//  breathCCval = map(constrain(breathLevel,ON_Thr,breath_max),ON_Thr,breath_max,0,127);
  ...
  analogWrite(breathLED, breathCCval*1.2);
    }
    if (breathAT){
      // send aftertouch
...
//**************************************************************
void pitch_bend(){
  pitchBend = analogRead(PitchBendPin); // read voltage on analog pin A0
//  pitchBend = 0;
  if (pitchBend > modsHi_Thr){
    pitchBend = oldpb*0.6+0.4*map(constrain(pitchBend,modsHi_Thr,modsMax),modsHi_Thr,modsMax,0,-8192); 
    // go from 8192 to 16383 (full pb up) when off center threshold going up
  } 
  else if (pitchBend < modsLo_Thr){
    pitchBend = oldpb*0.6+0.4*map(constrain(pitchBend,modsMin,modsLo_Thr),modsMin,modsLo_Thr,8192,0); 
    // go from 8192 to 0 (full pb dn) when off center threshold going down
  } 
  else {
...
//
void modulation(){
  int modRead = analogRead(JoyY); // read voltage on analog pin A6
//  int modRead= 0;
  if (modRead < modsLo_Thr+2000){
    modLevel = map(constrain(modRead,modsMin,modsLo_Thr),modsMin,modsLo_Thr,127,0); 
    // go from 0 to full modulation when off center threshold going left(?)
  } else {
...
void readSwitches(){
  ...
  //this will return true, or 1, if the touchRead measurement is greater than the touch_Thr, or false / 0 if it's lesser. Then, this 1 or 0 is used in the fingering equations below
  RH1=touchRead(18)>touch_Thr; //this will return true, or 1, if the touchRead measurement is greater than the touch_Thr, or false / 0 if it's lesser. Then, this 1 or 0 is used in the fingering equations below
  RH2=touchRead(0)>touch_Thr;
  RH3=touchRead(25)>touch_Thr;
  RH4=touchRead(1)>touch_Thr;
  TH1=touchRead(15)>touch_Thr;
  TH2=touchRead(16)>touch_Thr;
  TH3=touchRead(19)>touch_Thr;
  TH4=touchRead(17)>touch_Thr;
  OCTup=touchRead(32)>touch_Thr;
  OCTdn=touchRead(33)>touch_Thr;
  computePixels();
  modeSelector();
  pixels.show();   // Send the updated pixel colors to the hardware.
...
//  Serial.println(instrumentMode);
  ...
  switch (instrumentMode){
    case 0:// Tuba mode
      fingeredNote=constrain(startNote - (2*RH1 + RH2 + 3*RH3 + 5*RH4) + (7*TH1 + 5*TH2) - (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) 
      ,0, 127);
      break;
//    case 1: // One voice, lefty mode LH  is descending intervals, RH is ascending intervals
//      fingeredNote=constrain(startNote - (2*LH1) - (LH2) - 4*LH3 - (7*LH4) + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) - (5*TH3 + 7*TH4)  - (5*TH3 + 7*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) 
//      ,0, 127);
//      break;
//    case 2: //TWO INDEPENDENT VOICES
//      fingeredNote=constrain(startNote - ((2*RH1) + (RH2) + 4*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4)  + (12*OCTup) - (12*OCTdn) + (octave*12) //  Right hand
//      ,0, 127);
...
//      ,0, 127);
//      break;
//    case 3: //Duet RH +LH fingers - 1 oct
//      fingeredNote=constrain(startNote - ((2*RH1) + (RH2) + 3*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) // Right hand
//      ,0, 127);
//      fingeredNoteL=constrain(fingeredNote + ((2*LH1) + (LH2) + 4*LH3 + (7*LH4)) -12 // Left hand
//      ,0, 127);
//      break;
//    case 4://Duet RH +LH fingers -2 oct
//      fingeredNote=constrain(startNote - ((2*RH1) + (RH2) + 3*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) // Right hand
//      ,0, 127);
//      fingeredNoteL=constrain(fingeredNote + ((2*LH1) + (LH2) + 4*LH3 + (7*LH4)) -24 // Left hand
//      ,0, 127);
//      break;
//    case 5: //Duet RH +LH fingers
//      fingeredNote=constrain(startNote - ((2*RH1) + (RH2) + 3*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) // Right hand
//      ,0, 127);
...
//      break;
//    case 6: // B flat drone
//      fingeredNote=constrain(fingeredNote=startNote + (2*LH1) + (LH2) + 3*LH3 + (7*LH4) - ((2*RH1) + (RH2) + 3*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) // Both hands
//      ,0, 127);
//      fingeredNoteL=constrain(startNote-12 // Drone note
//      ,0, 127);
//      break;
//    case 7: // Close harmony, easy thirds 
//      fingeredNote=constrain(startNote - ((2*RH1) + (RH2) + 3*RH3 + (5*RH4) + 2*RH5 + RH6) - (5*TH1 + 7*TH2) + (7*TH3 + 5*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12) // Right hand
//      ,0, 127);
...
// OLD MODE Equations
//    case 2:
//      fingeredNote=startNote + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4) + 2*RH5 + RH6) + (7*TH1 + 5*TH2) - (5*TH3 + 7*TH4)  + (12*OCTup) - (12*OCTdn) + (octave*12); // Right hand
//      fingeredNoteL=startNote + ((2*LH1) + (LH2) + 4*LH3 + (7*LH4)) + (7*TH1 + 5*TH2) - (5*TH3 + 7*TH4)  + (12*OCTup) - (12*OCTdn) + (octave*12) -12; // Left hand
//      break;
//    case 3:
//      fingeredNote=startNote + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4) + 2*RH5 + RH6) + (7*TH1 + 5*TH2) - (5*TH3 + 7*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12); // Right hand
...
//      break;
//    case 6:
//      fingeredNote=fingeredNote=startNote - (2*LH1) - (LH2) - 4*LH3 - (7*LH4) + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4) + 2*RH5 + RH6) + (7*TH1 + 5*TH2) - (5*TH3 + 7*TH4) + (12*OCTup) - (12*OCTdn) + (octave*12); // Option 1
//      fingeredNoteL=startNote-24; // Left hand
//      break;
...
void readPots(){
  ...
  int pot1Value=map(pot1Reading, 5, 4095, 127, 0 );
  if (abs(pot1Value - oldPot1) > potDeadZone){
    if (millis() - pot1_change_time > ON_Delay) {
    usbMIDI.sendControlChange(16, pot1Value, MIDIchannel);
    oldPot1=pot1Value;
    }
  }
  int pot2Reading = analogRead(pot2pin);
  int pot2Value=map(pot2Reading, 5, 4095, 127, 0 );
  if (abs(pot2Value - oldPot2) > potDeadZone){
    if (millis() - pot2_change_time > ON_Delay) {
    usbMIDI.sendControlChange(7, pot2Value, MIDIchannel);
    oldPot2=pot2Value;
    }
  }
  int FSRReading = analogRead(ForceSensitiveResistor);
  int FSRValue=map(FSRReading, 5, 4095, 0, 127 );
  if (abs(FSRValue - oldFSR) > potDeadZone){
    if (millis() - FSR_change_time > ON_Delay) {
    usbMIDI.sendControlChange(1, FSRValue, MIDIchannel);
    FSRPot=FSRValue;
    }
  }
}
