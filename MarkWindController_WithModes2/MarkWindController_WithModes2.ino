...
#define ON_Delay   40   // Set Delay after ON threshold before velocity is checked (wait for tounging peak)
#define breath_max 4095 // Threshold for maximum breath
#define modsLo_Thr 2800 // Low threshold for mod stick center
#define modsHi_Thr 3200 // High threshold for mod stick center
...
void mainLoop() {  
  ...
  while(1){
    ...
    if (MidChUpDebo.fell()){ //using Midi channel down for mode selection at the moment
      usbMIDI.sendControlChange(126, 2, MIDIchannel);
      if (instrumentMode != 0){
        usbMIDI.sendControlChange(126, 2, (MIDIchannel+1));
      }
...
void pitch_bend(){
  ...
  if (pitchBend > modsHi_Thr){
    int constrainedPitchBend = constrain(pitchBend,modsHi_Thr,modsMax);
    pitchBend = oldpb*0.6+0.4*map(constrainedPitchBend,modsHi_Thr,modsMax,4096,8192); // go from 8192 to 16383 (full pb up) when off center threshold going up
  } else if (pitchBend < modsLo_Thr){
    int constrainedPitchBend = constrain(pitchBend,modsMin,modsLo_Thr);
    pitchBend = oldpb*0.6+0.4*map(constrainedPitchBend,modsMin,modsLo_Thr,0,4096); // go from 8192 to 0 (full pb dn) when off center threshold going down
//    pitchBend = 8192;
  } else {
    pitchBend = oldpb*0.6+8192*0.4; // released, so smooth your way back to zero
    if ((pitchBend > 4086) && (pitchBend < 5009)) pitchBend = 8192; // 8192 is 0 pitch bend, don't miss it bc of smoothing
  }
  if (mod || (ccList[breathCC]==1)){
//    if (PB) pitchBend += lfoLevel/6; else pitchBend += lfoLevel;
    pitchBend=constrain(pitchBend, 0, 8192);
  }
...
void modulation(){
 ...
 // int modRead = 0;
  if (modRead < modsLo_Thr){
    int constrainedModRead = constrain(modRead,modsMin,modsLo_Thr);
    modLevel = map(constrainedModRead,modsMin,modsLo_Thr,127,0); // go from 0 to full modulation when off center threshold going left(?)
  } else {
...
//  Serial.println(instrumentMode);
  ...
  switch (instrumentMode){
    case 0:
      fingeredNote=startNote - (2*LH1) - (LH2) - 4*LH3 - (7*LH4) + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4)) + (12*OCTup) - (12*OCTdn) + (octave*12) + transposition; // Option 1
      pixels.setPixelColor(instrumentMode, 20, 50, 0);
      break;
    case 1:
      fingeredNote=startNote + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4)) + (12*OCTup) - (12*OCTdn) + (octave*12) + transposition; // Right hand
      fingeredNoteL=startNote + ((2*LH1) + (LH2) + 4*LH3 + (7*LH4)) + (12*OCTup) - (12*OCTdn) + (octave*12) + transposition -12; // Left hand
      break;
    case 2:
      fingeredNote=startNote + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4)) + (12*OCTup) - (12*OCTdn) + (octave*12) + transposition; // Right hand
    ...
    case 4:
      ...
      break;
    case 5:
      fingeredNote=fingeredNote=startNote - (2*LH1) - (LH2) - 4*LH3 - (7*LH4) + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4)) + (12*OCTup) - (12*OCTdn) + (octave*12) + transposition; // Option 1
