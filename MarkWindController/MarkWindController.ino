...
byte LH4;  // Left Hand pinky key 1 (pitch change +1)
...
byte RH4;  // Right Hand pinky key 2 (pitch change -1)
//byte RHp3;  // Right Hand pinky key 3 (pitch change -2, in this version -1 if RH4 is pressed)
...
void readSwitches(){  
 ...
 // RHp3=touchRead(0)>touch_Thr;
  ...
  fingeredNote=startNote - (2*LH1) - (LH2) - 4*LH3 - (7*LH4) + ((2*RH1) + (RH2) + 4*RH3 + (7*RH4)) + (12*OCTup) - (12*OCTdn); // Option 1
