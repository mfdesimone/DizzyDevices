...
void loop() { // main program loop
  ...
  if (pageMode == 0) { // if we are not currently selecting a page
    ...
    noteOn(0x90, 0x44, 0x45);
    delay(100);
    //Note on channel 1 (0x90), some note value (note), silent velocity (0x00):
    noteOn(0x90, 0x44, 0x00);
    delay(100);
      //Serial.println("b_0");
    }
...
void noteOff(int cmd, int pitch, int velocity) {
  ...
  Serial.write(pitch); // pitch #, in hexadecimal
  Serial.write(velocity); //sending a note on command with velocity zero is the same as a note off command
}
void noteOn(int cmd, int pitch, int velocity) {
  Serial.write(cmd);
  Serial.write(pitch);
  Serial.write(velocity); //0x45 is middle velocity
}
