...
//      Serial.println(state);
    readButtons();
    pressureSensor = breathFilter.input(analogRead(breathPin));
    sensorNeedle = pressureSensor - sensorMidpoint;
...
//      if (pressureSensor > ON_Thr) {
      if ((abs(sensorNeedle) > breathDeadZone)){
        ...
        if (millis() - breath_on_time > ON_Delay ) {
          ...
          if (sensorNeedle < 0){ // we're blowing
            int sensorNeedleLowBoundary = sensorMidpoint + (breathDeadZone / 2);
            Serial.println("+");
            breathLevel=constrain(pressureSensor, breath_min, sensorNeedleLowBoundary);
            breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_min, 0, 127));
//            Serial.println(breathLevel);
          }
          if (sensorNeedle > 0) { // we're sucking
            int sensorNeedleLowBoundary = sensorMidpoint - (breathDeadZone / 2);
            Serial.println("-");
            breathLevel=constrain(max(pressureSensor, initial_breath_value), sensorNeedleLowBoundary, breath_max);
//            Serial.println(breathLevel);
            breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_max, 0, 127));
          }
          velocitySend = constrain(breathValHires,1,127);
...
//          Serial.println(fingeredNote);
        ...
        // than the debounce delay, so take it as the actual current state
          if (fingeredNote != activeNote) {
            ...
            if (sensorNeedle < 0){ // we're blowing
              int sensorNeedleLowBoundary = sensorMidpoint - (breathDeadZone / 2);
//              breathLevel=constrain(min(pressureSensor, initial_breath_value), sensorNeedleLowBoundary, breath_min);
              breathValHires = breathCurve(map(constrain(breathLevel,breath_min,sensorMidpoint), sensorNeedleLowBoundary, breath_min, 0, 16383));
            }
            if (sensorNeedle > 0) { // we're sucking
              int sensorNeedleLowBoundary = sensorMidpoint + (breathDeadZone / 2);
//              breathLevel=constrain(min(pressureSensor,initial_breath_value), breath_max, sensorNeedleLowBoundary);
              breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_max, 0, 16383));
            }
...
//            Serial.println(velocitySend);
          ...
          if (fingeredNoteL != activeNoteL) {
            ...
            if (sensorNeedle < 0){ // we're blowing
              int sensorNeedleLowBoundary = sensorMidpoint - (breathDeadZone / 2);
//              breathLevel=constrain(min(pressureSensor, initial_breath_value), sensorNeedleLowBoundary, breath_min);
              breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_min, 0, 16383));
            }
            if (sensorNeedle > 0) { // we're sucking
              int sensorNeedleLowBoundary = sensorMidpoint + (breathDeadZone / 2);
//                breathLevel=constrain(max(pressureSensor,initial_breath_value), breath_max, sensorNeedleLowBoundary);
              breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_max, 0, 16383));
            }
            if (sensorNeedle < 0){ // we're blowing
              int sensorNeedleLowBoundary = sensorMidpoint + (breathDeadZone / 2);
              Serial.println("+");
              breathLevel=constrain(min(pressureSensor, initial_breath_value), breath_min, sensorNeedleLowBoundary);
              breathValHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_min, 0, 16383));
            }
            if (sensorNeedle > 0) { // we're sucking
              int sensorNeedleLowBoundary = sensorMidpoint - (breathDeadZone / 2);
...
void breath(){
  int breathCCval;
  unsigned int breathCCvalHires =0;
  ...
  if (sensorNeedle < 0){ // we're blowing
    int sensorNeedleLowBoundary = sensorMidpoint - (breathDeadZone / 2);
//    Serial.println("+");
    breathLevel=constrain(pressureSensor, 0, sensorNeedleLowBoundary);
    breathCCvalHires = breathCurve(map(breathLevel, breath_min, sensorNeedleLowBoundary, 127, 0));
  }
  if (sensorNeedle > 0) { // we're sucking
//    Serial.println("-");
    int sensorNeedleLowBoundary = sensorMidpoint + (breathDeadZone / 2);
    breathLevel=constrain(pressureSensor, sensorNeedleLowBoundary, breath_max);
//    Serial.println(breathLevel);
    breathCCvalHires = breathCurve(map(breathLevel, sensorNeedleLowBoundary, breath_max, 0, 127));
  }
  breathCCval = constrain(breathCCvalHires, 0, 127) ;
