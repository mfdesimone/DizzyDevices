...
void readSwitches(){  
  ...
  if (cap.touched() & (1 << 7)){LH4=1;} // this should be a true or false, or 1 or 0. The MPR121 board does the threshold stuff automatically, and just returns a 16 bit number when we use cap.touched(). The lower 12 bits of that number correspond to each of the 12 touchpads
  if (cap.touched() & (1 << 4)){RH5=1;}
  if (cap.touched() & (1 << 6)){RH1=1;}
  if (cap.touched() & (1 << 5)){RH6=1;}
...
void readPots(){
  int pot1Reading = analogRead(A14);
  
  
  Serial.println(pot1Reading);
 
    int pot1Value=constrain(pot1Reading, 3000, 4500);
    int factor =0.4;
    if (pot1Value < 4000) {
      factor =0.1;
    }
    pot1Value = oldcc16*0.6+0.4*map(pot1Value, 3000, 4500, 0, 127);
  } 
  else {
    pot1Value = 0;
  }
  if (pot1Value != oldcc16){// only send midi data if cc16 has changed from previous value
